    ...
    public class ClubStatusController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "club_sts,club_status_desc")] ClubStatus clubStatus)
        {
            if (ModelState.IsValid)
            {
                ...
                writeLog.TransactionLog(identity.User.member_seq, "CreateClubStatus", identity.User.club_id);
        ...
        public ActionResult Edit([Bind(Include = "club_sts,club_status_desc")] ClubStatus clubStatus)
        {
            if (ModelState.IsValid)
            {
                ...
                // Write log to table TransactionLog
                WriteLog writeLog = new WriteLog();
                writeLog.TransactionLog(identity.User.member_seq, "EditClubStatus", identity.User.club_id);
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(clubStatus);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            // Write log to table TransactionLog
            WriteLog writeLog = new WriteLog();
            writeLog.TransactionLog(identity.User.member_seq, "DeleteClubStatus", identity.User.club_id);
            return RedirectToAction("Index");
        }
